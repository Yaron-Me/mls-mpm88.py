...
# create figure with 1 plot not with fig,axs
fig, axs = plt.subplots()


for i, path in enumerate(paths):
    times, _, _ = readFile(path)

    color = 'go' if i % 2 == 0 else 'yo'

    n = float(names[i // 2])
    finaltime = times[-1]

    # plot final with n on x-axis
    axs.plot(n, finaltime, color)

    axs.set_xlabel('Tensile strength [Pa]', fontsize=12)
    axs.set_ylabel('Time to settle [s]', fontsize=12)
    axs.set_title('Time it takes for the beam to settle vs tensile strength for \n\
both random and grid-like particle placement.')
...
# set grid
axs.grid()
axs.legend(['Random placement', 'Grid-like placement'])
# axs[1].grid()
